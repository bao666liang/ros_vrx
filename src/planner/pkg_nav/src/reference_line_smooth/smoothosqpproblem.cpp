...
bool FemPosDeviationSqpOsqpInterface::Solve()
{
    ...
    if (ref_points_.size() != mUpperBounds_around_refs.size() || ref_points_.size() != mLowerBounds_around_refs.size())
    {
        std::cout << "ref_points and bounds size not equal, solver early terminates";
        return false;
    }
    ...
    num_of_points_ = static_cast<int>(ref_points_.size());               // 参考点的个数
    num_of_pos_variables_ = num_of_points_ * 2;                          // 变量点的个数 = 参考点的个数*2
    num_of_slack_variables_ = num_of_points_ - 2;                        // 松弛变量的个数 = 参考点的个数-2
    num_of_variables_ = num_of_pos_variables_ + num_of_slack_variables_; // 总的变量个数 = 变量点的个数 + 松弛变量的个数

    num_of_variable_constraints_ = num_of_variables_;   // 变量的约束个数
    num_of_curvature_constraints_ = num_of_points_ - 2; // 曲率的约束个数
    num_of_constraints_ =
        num_of_variable_constraints_ + num_of_curvature_constraints_; // 总的约束个数 = 变量的约束个数 + 曲率的约束个数
    ...
    std::vector<c_float> *upper_bounds)
{
    ...
    (*upper_bounds)[0 * 2] = ref_point_xy.first + mUpperBounds_around_refs[0].first; ////cyb
    // (*upper_bounds)[0 * 2 + 1] = ref_point_xy.second - 25.0;   // 这里的-25，应该是为了适应特定的参考线，为了通用应该是下面这句（ymj add）
    // (*upper_bounds)[0 * 2 + 1] = ref_point_xy.second + bounds_around_refs_[0]; 
    (*upper_bounds)[0 * 2 + 1] = ref_point_xy.second + mUpperBounds_around_refs[0].second; //cyb
    // (*lower_bounds)[0 * 2] = ref_point_xy.first - bounds_around_refs_[0];
    (*lower_bounds)[0 * 2] = ref_point_xy.first - mLowerBounds_around_refs[0].first; //cyb
    // (*lower_bounds)[0 * 2 + 1] = ref_point_xy.second - 26.0;  // 这里同理
    // (*lower_bounds)[0 * 2 + 1] = ref_point_xy.second - bounds_around_refs_[0];
    (*lower_bounds)[0 * 2 + 1] = ref_point_xy.second - mLowerBounds_around_refs[0].second; //cyb

    for (int i = 1; i < num_of_points_; ++i)  //cyb 
    {
        const auto &ref_point_xy = ref_points_[i];
        (*upper_bounds)[i * 2] = ref_point_xy.first + mUpperBounds_around_refs[i].first;
        (*upper_bounds)[i * 2 + 1] = ref_point_xy.second + mUpperBounds_around_refs[i].second;
        (*lower_bounds)[i * 2] = ref_point_xy.first - mLowerBounds_around_refs[i].first;
        (*lower_bounds)[i * 2 + 1] = ref_point_xy.second - mLowerBounds_around_refs[i].second;
    }
