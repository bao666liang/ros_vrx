...
public:
    ...
    void set_bounds_around_refs(std::vector<std::pair<double, double>> upperBounds_around_refs, std::vector<std::pair<double, double>> lowerBounds_around_refs)
    {
        mUpperBounds_around_refs = upperBounds_around_refs;
        mLowerBounds_around_refs = lowerBounds_around_refs;
    }
...
private:
    ...
    double curvature_constraint_ = 0.05; //0.15;

    // Weights in optimization cost function
    double weight_fem_pos_deviation_ = 1.0e7; //第1部分——参考点距离代价  //1.0e5;
    double weight_path_length_ = 1;  // 1
    double weight_ref_deviation_ = 1; //第2部分——平滑性代价  //10
