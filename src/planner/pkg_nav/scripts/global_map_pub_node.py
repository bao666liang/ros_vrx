...
def DataUpdating(map_pub, local_map, map_height, map_width):
    ...
    local_map.info.resolution = 1  # 分辨率 1cm  图片分片率(单位: m/像素)。
...
def node():
    ...
    try:
        # 初始化节点MapPublish
        rospy.init_node('ow_global_map_publish_node')
        # 定义发布器 map_pub 发布 local_map
        map_pub = rospy.Publisher(
            '/ow/global_map', OccupancyGrid, queue_size=1)
        ...
        rospy.loginfo("The ow_global_map_publish_node is running ...")
        # 初始化循环频率
        rate = rospy.Rate(1)
        # 在程序没退出的情况下
